...
(function () {
  ...
  /* hero cinematic core: beam → pulsing core → refracted fanning rays */
  var core=document.querySelector('.hero-core');
  if(core && core.getContext){
    var cx2=core.getContext('2d');
    var CDPR=Math.min(window.devicePixelRatio||1,2), CW=0, CH=0;
    var sizeCore=function(){ CW=core.clientWidth; CH=core.clientHeight; core.width=Math.max(1,CW*CDPR); core.height=Math.max(1,CH*CDPR); cx2.setTransform(CDPR,0,0,CDPR,0,0); };
    sizeCore(); window.addEventListener('resize',sizeCore,{passive:true});
    var RAYS=9;
    var drawCore=function(s){
      cx2.clearRect(0,0,CW,CH);
      var ox=CW*0.58, oy=CH*0.5;            /* core position */
      var span=Math.min(CW,CH);
      cx2.globalCompositeOperation='lighter';
      /* incoming beam from left edge into core */
      var beam=cx2.createLinearGradient(0,oy,ox,oy);
      beam.addColorStop(0,'rgba(43,232,165,0)');
      beam.addColorStop(1,'rgba(124,247,199,.55)');
      cx2.strokeStyle=beam; cx2.lineWidth=2.2;
      cx2.beginPath(); cx2.moveTo(0,oy); cx2.lineTo(ox,oy); cx2.stroke();
      /* travelling pulse along beam */
      var bp=(s*0.18)%1, bx=bp*ox;
      var pg=cx2.createRadialGradient(bx,oy,0,bx,oy,16);
      pg.addColorStop(0,'rgba(124,247,199,.7)'); pg.addColorStop(1,'rgba(124,247,199,0)');
      cx2.fillStyle=pg; cx2.beginPath(); cx2.arc(bx,oy,16,0,6.2832); cx2.fill();
      /* refracted rays fanning out to the right */
      var spread=1.05;                       /* total fan angle (rad) */
      for(var r=0;r<RAYS;r++){
        var t=RAYS>1?r/(RAYS-1):0.5;
        var ang=(t-0.5)*spread;
        var len=span*0.5*(0.7+0.3*Math.cos(ang));
        var ex=ox+Math.cos(ang)*len, ey=oy+Math.sin(ang)*len;
        var a=0.10+0.16*(0.5+0.5*Math.sin(s*1.1+r*0.9));
        var rg=cx2.createLinearGradient(ox,oy,ex,ey);
        rg.addColorStop(0,'rgba(43,232,165,'+(a+0.12).toFixed(3)+')');
        rg.addColorStop(1,'rgba(43,232,165,0)');
        cx2.strokeStyle=rg; cx2.lineWidth=1.4;
        cx2.beginPath(); cx2.moveTo(ox,oy); cx2.lineTo(ex,ey); cx2.stroke();
      }
      /* orbiting nodes */
      for(var n=0;n<6;n++){
        var na=s*0.5+n*1.047, nr=span*0.2*(0.7+0.3*Math.sin(s*0.7+n));
        var nx=ox+Math.cos(na)*nr*1.15, ny=oy+Math.sin(na)*nr*0.8;
        cx2.fillStyle='rgba(124,247,199,'+(0.35+0.25*Math.sin(s+n)).toFixed(3)+')';
        cx2.beginPath(); cx2.arc(nx,ny,1.8,0,6.2832); cx2.fill();
      }
      /* glowing core */
      var pulse=1+0.07*Math.sin(s*1.6);
      var cr=span*0.13*pulse;
      var halo=cx2.createRadialGradient(ox,oy,0,ox,oy,cr*2.6);
      halo.addColorStop(0,'rgba(43,232,165,.5)');
      halo.addColorStop(0.4,'rgba(43,232,165,.18)');
      halo.addColorStop(1,'rgba(43,232,165,0)');
      cx2.fillStyle=halo; cx2.beginPath(); cx2.arc(ox,oy,cr*2.6,0,6.2832); cx2.fill();
      var hot=cx2.createRadialGradient(ox,oy,0,ox,oy,cr);
      hot.addColorStop(0,'rgba(240,255,250,.95)');
      hot.addColorStop(0.5,'rgba(124,247,199,.6)');
      hot.addColorStop(1,'rgba(43,232,165,0)');
      cx2.fillStyle=hot; cx2.beginPath(); cx2.arc(ox,oy,cr,0,6.2832); cx2.fill();
      cx2.globalCompositeOperation='source-over';
    };
    if(reduce){ drawCore(0); }
    else{
      var clast=0;
      var cframe=function(ts){ requestAnimationFrame(cframe); if(document.hidden||ts-clast<33) return; clast=ts; drawCore(ts/1000); };
