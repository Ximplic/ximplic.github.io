    ...
    <>
      ...
      <section className="px-6 pt-32 md:pt-36 pb-16">
        <div className="max-w-7xl mx-auto rv">
          ...
          <h1 className="text-4xl md:text-6xl font-semibold tracking-tight max-w-4xl leading-[1.05] mb-6" style={{ letterSpacing: "-0.04em" }}>
      ...
      <section className="px-6 py-24 border-t border-white/[0.06]">
        <div className="max-w-7xl mx-auto rv">
          ...
          <h2 className="text-3xl md:text-4xl font-semibold tracking-tight mb-4 max-w-2xl">One accelerator, the whole path</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { tag: "Launching now", name: "Vyzora", desc: "The Virtual Prototype. SystemC model, run any model and measure power, no hardware.", to: "/products/vyzora" },
              ...
              <div key={p.name} className={`rounded-xl border border-white/10 bg-white/[0.02] p-6 flex flex-col transition-colors hover:border-white/20 ${p.to ? "" : "opacity-60"}`}>
                <span className="text-[10px] font-medium tracking-widest uppercase text-white/40 mb-4">{p.tag}</span>
                <h3 className="text-xl font-semibold mb-2">{p.name}</h3>
                <p className="text-sm text-white/55 flex-1 leading-relaxed">{p.desc}</p>
                {p.to
                  ? <Link to={p.to} className="text-sm font-medium mt-5 inline-flex items-center gap-1" style={{ color: TEAL }}>Explore {p.name} <ArrowRight size={13} /></Link>
                  : <span className="text-xs uppercase tracking-wider text-white/35 mt-5">Coming soon</span>}
              </div>
            ))}
          </div>
          <p className="mt-10">
            <a href="#contact" className="text-sm font-medium inline-flex items-center gap-1" style={{ color: TEAL }}>See how to adopt it <ArrowRight size={13} /></a>
          </p>
        </div>
      </section>
      ...
      <section id="contact" className="px-6 py-28 border-t border-white/[0.06]">
        <div className="max-w-2xl mx-auto text-center rv">
          ...
          <h2 className="text-3xl md:text-5xl font-semibold tracking-tight mb-5" style={{ letterSpacing: "-0.03em" }}>Find the right starting point</h2>
